...
class Console {
    ...
    isShowLog(...data) {
        ...
        if (isShowGlobalLog) {
            // 如果开启全局开关，把参数给分个打印出来
            if (!this.timeStampArr[this.timeStamp]) {
    ...
    log(...dataArr) {
        // 获取当前时间戳
        this.timeStamp = new Date().getTime() + `dsx2016`
        let nowTome = this.timeStamp
        this.timeStampArr[nowTome] = true
        // 获取传递来的参数
        this.data = [...dataArr]
        this.timeStampArr[nowTome+`timer`] = setTimeout(() => {
            if (this.timeStampArr[nowTome]) {
    ...
    toString(...data) {
        ...
        this.timeStampArr[this.timeStamp] = false
        this.format()
        this.isShowLog()
    }
