...
let initLogStyle=(...dataArr)=>{
    // 创建一个实例
    let dsxConsole = new Console()
    dsxConsole.logStyle.apply(dsxConsole,dataArr)
    let timer = setTimeout(() => {
        dsxConsole = null
        clearTimeout(timer)
    }, 100)
...
let initLog = (...dataArr) => {
    // 创建一个实例
    let dsxConsole = new Console()
    dsxConsole.log.apply(dsxConsole,dataArr)
    let timer = setTimeout(() => {
        dsxConsole = null
        clearTimeout(timer)
    }, 100)
...
export let Log = (...dataArr) => {
    return funcName(`log`,dataArr)
}

// 设置log方法
export let LogStyle = (...dataArr) => {
    return funcName(`logStyle`,dataArr)
}
